...
var r = function(r2) {
  const a4 = __spreadValues(__spreadValues({}, t), "string" == typeof r2 ? { url: r2 } : r2);
...
var l = class {
  ...
  send(e2) {
    const t2 = "arraybuffer" !== this.responseType, r2 = __spreadProps(__spreadValues({}, this.#s), { method: this.#e, headers: this.#o, body: e2 });
...
var Transaction = class _Transaction extends BaseTransaction {
  ...
  patch(patchOrDocumentId, patchOps) {
    ...
    return this._add({ patch: __spreadValues({ id: patchOrDocumentId }, patchOps) });
  }
};
...
var encodeQueryString = ({
  ...
  const searchParams = new URLSearchParams(), _a = options, { tag, includeMutations, returnQuery } = _a, opts = __objRest(_a, ["tag", "includeMutations", "returnQuery"]);
...
function _fetch(client, httpRequest, _stega, query, _params = {}, options = {}) {
  const stega = "stega" in options ? __spreadValues(__spreadValues({}, _stega || {}), typeof options.stega == "boolean" ? { enabled: options.stega } : options.stega || {}) : _stega, params = stega.enabled ? stegaClean(_params) : _params, mapResponse = options.filterResponse === false ? (res) => res : (res) => res.result, _a = __spreadProps(__spreadValues({
  ...
  }), { cache, next } = _a, opts = __objRest(_a, ["cache", "next"]), reqOpts = typeof cache < "u" || typeof next < "u" ? __spreadProps(__spreadValues({}, opts), { fetch: { cache, next } }) : opts, $request = _dataRequest(client, httpRequest, "query", { query, params }, reqOpts);
  return stega.enabled ? $request.pipe(
    ...
    map(
      ([res, stegaEncodeSourceMap]) => {
        ...
        return mapResponse(__spreadProps(__spreadValues({}, res), { result }));
      }
    )
...
function _requestObservable(client, httpRequest, options) {
  ...
  if (tag && options.tag !== null && (options.query = __spreadValues({ tag: requestTag(tag) }, options.query)), ["GET", "HEAD", "POST"].indexOf(options.method || "GET") >= 0 && isQuery(client, uri)) {
    const resultSourceMap = options.resultSourceMap ?? config.resultSourceMap;
    resultSourceMap !== void 0 && resultSourceMap !== false && (options.query = __spreadValues({ resultSourceMap }, options.query));
    ...
    perspectiveOption === "previewDrafts" || perspectiveOption === "drafts") && useCdn && (useCdn = false, printCdnPreviewDraftsWarning2())), options.lastLiveEventId && (options.query = __spreadProps(__spreadValues({}, options.query), { lastLiveEventId: options.lastLiveEventId })), options.returnQuery === false && (options.query = __spreadValues({ returnQuery: "false" }, options.query)), useCdn && options.cacheMode == "noStale" && (options.query = __spreadValues({ cacheMode: "noStale" }, options.query));
  }
...
var eventSourcePolyfill = defer(() => import("./browser-P7F63RJ5.js")).pipe(
...
function _listen(query, params, opts = {}) {
  const { url, token, withCredentials, requestTagPrefix, headers: configHeaders } = this.config(), tag = opts.tag && requestTagPrefix ? [requestTagPrefix, opts.tag].join(".") : opts.tag, options = __spreadProps(__spreadValues({}, defaults(opts, defaultOptions)), { tag }), listenOpts = pick(options, possibleOptions), qs = encodeQueryString({ query, params, options: __spreadValues({ tag }, listenOpts) }), uri = `${url}${_getDataUrl(this, "listen", qs)}`;
...
function shareReplayLatest(configOrPredicate, config) {
  return _shareReplayLatest(
    typeof configOrPredicate == "function" ? __spreadValues({ predicate: configOrPredicate }, config) : configOrPredicate
  );
}
function _shareReplayLatest(config) {
  return (source) => {
    let latest, emitted = false;
    const _a = config, { predicate } = _a, shareConfig = __objRest(_a, ["predicate"]), wrapped = source.pipe(
      tap((value) => {
    ...
    const events = connectEventSource(() => (
      ...
      map((event) => {
        if (event.type === "message") {
          const _a = event, { data } = _a, rest = __objRest(_a, ["data"]);
          return __spreadProps(__spreadValues({}, rest), { tags: data.tags });
        }
        return event;
      })
    ).pipe(
...
function fetchObservable(url, init) {
  return new Observable((observer) => {
    ...
    return fetch(url, __spreadProps(__spreadValues({}, init), { signal: controller.signal })).then(
      (response) => {
...
var ObservableReleasesClient = class {
  ...
  create(releaseOrOptions, maybeOptions) {
    ...
    return _action(this.#client, this.#httpRequest, action, options).pipe(
      map((actionResult) => __spreadProps(__spreadValues({}, actionResult), {
        releaseId,
        metadata
      }))
    );
...
var ReleasesClient = class {
  ...
  create(releaseOrOptions, maybeOptions) {
    return __async(this, null, function* () {
      const { action, options } = createRelease(releaseOrOptions, maybeOptions), { releaseId, metadata } = action;
      return __spreadProps(__spreadValues({}, yield lastValueFrom(
...
var ObservableSanityClient = class _ObservableSanityClient {
  ...
  config(newConfig) {
    if (newConfig === void 0)
      return __spreadValues({}, this.#clientConfig);
  ...
  withConfig(newConfig) {
    ...
    return new _ObservableSanityClient(this.#httpRequest, __spreadProps(__spreadValues(__spreadValues({}, thisConfig), newConfig), {
      stega: __spreadValues(__spreadValues({}, thisConfig.stega || {}), typeof newConfig?.stega == "boolean" ? { enabled: newConfig.stega } : newConfig?.stega || {})
    }));
  }
  ...
  getDocument(id, options) {
    if (options?.includeAllVersions === true)
      return _getDocument(this, this.#httpRequest, id, __spreadProps(__spreadValues({}, options), {
  ...
  createVersion({
    ...
    const documentVersionId = deriveDocumentVersionId("createVersion", {
      ...
      publishedId,
      releaseId
    }), documentVersion = __spreadProps(__spreadValues({}, document2), { _id: documentVersionId }), versionPublishedId = publishedId || getPublishedId(document2._id);
  ...
  replaceVersion({
    ...
    const documentVersionId = deriveDocumentVersionId("replaceVersion", {
      ...
      publishedId,
      releaseId
    }), documentVersion = __spreadProps(__spreadValues({}, document2), { _id: documentVersionId });
...
var SanityClient = class _SanityClient {
  ...
  getDocument(id, options) {
    if (options?.includeAllVersions === true)
      return lastValueFrom(
        _getDocument(this, this.#httpRequest, id, __spreadProps(__spreadValues({}, options), {
          includeAllVersions: true
        }))
      );
    const opts = __spreadValues({
...
function defineCreateClientExports(envMiddleware2, ClassConstructor) {
  return { requester: defineHttpRequest(envMiddleware2), createClient: (config) => {
    ...
    return new ClassConstructor(
      (options, requester2) => (requester2 || clientRequester)(__spreadValues({
        maxRedirects: 0,
